...
if penguin_file is None:
    rf_pickle = open(
        "/workspaces/ughhhh/streamlit-app/penguin_ml/random_forest_penguin.pickle", "rb"
    )
    map_pickle = open(
        "/workspaces/ughhhh/streamlit-app/penguin_ml/output_penguin.pickle", "rb"
    )
...
with st.form("user_inputs"):
    ...
    st.write(
        f"We predict your penguin is of the {prediction_species} species"
    )
...
st.write(print(penguin_file.head()))
...
st.pyplot(ax)
fig, ax = plt.subplots()
ax = sns.distplot(x=penguin_file["flipper_length_mm"], hue=penguin_file["species"])
plt.axvline(flipper_length)
plt.title("Flipper Length by Species")
st.pyplot(ax)
